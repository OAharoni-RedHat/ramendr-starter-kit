...
@pytest.mark.parametrize("openshift_dyn_client", ["VP_HUBCONFIG"], indirect=True)
def test_subscription_status_hub(openshift_dyn_client):
    expected_subs = {
        ...
        # odf-multicluster-orchestrator installs into openshift-operators (cluster-wide)
        "odf-multicluster-orchestrator": ["openshift-operators"],
        # cert-manager is managed by the RH operator (openshift-cert-manager-operator)
...
@pytest.mark.parametrize(
    ...
    ["VP_OCP_PRIMARY", "VP_OCP_SECONDARY"],
    indirect=True,
)
...
def test_pod_status_hub(openshift_dyn_client):
    # odf-multicluster-orchestrator and ramen DR hub pods live in openshift-operators
    # (cluster-wide namespace); we check that namespace for the whole operator ecosystem
    projects = [
        "patterns-operator",
...
def test_pod_status_spoke(openshift_dyn_client):
    # openshift-storage is intentionally excluded: ODF spawns short-lived CronJob pods
    # (storageclient-*-status-reporter-*) that get garbage-collected between the list and
    # the per-pod GET that the library's skip_check logic performs, causing a spurious 404.
    # ODF health on the spokes is covered by test_subscription_status_spoke (odf-operator)
    # and end-to-end by test_drpc_available_and_peer_ready (working storage replication).
    projects = [
...
def test_managed_clusters(openshift_dyn_client):
    ...
    components.assert_managed_clusters(
        openshift_dyn_client, ["ocp-primary", "ocp-secondary"]
    )
...
@pytest.mark.parametrize(
    "openshift_dyn_client",
    ["VP_HUBCONFIG", "VP_OCP_PRIMARY", "VP_OCP_SECONDARY"],
    indirect=True,
)
...
def test_argocd_applications_health_hub(openshift_dyn_client):
    # vp-gitops holds the top-level hub app; ramendr-starter-kit-odf holds component apps.
    # vp-manage-proxy-cluster-ca OutOfSync was caused by ESO schema defaults (nullBytePolicy,
    # deletionPolicy, engineVersion, mergePolicy) not emitted by the chart; fixed via
...
def test_drpolicy_validated(openshift_dyn_client):
    ...
    assert not failures, "DRPolicy validation failures:\n" + "\n".join(failures)


@pytest.mark.parametrize("openshift_dyn_client", ["VP_HUBCONFIG"], indirect=True)
def test_drcluster_available(openshift_dyn_client):
    """All DRCluster objects must be phase=Available, Validated=True, and not Fenced."""
    ...
    for c in clusters:
        ...
        if not _condition_status(conditions, "Validated"):
            ...
            reason = cond.get("reason", "condition missing") if cond else "condition missing"
            errors.append(f"Validated != True ({reason})")
        # Fenced=True means the cluster is intentionally isolated for DR testing; not expected in steady state
...
def test_drpc_available_and_peer_ready(openshift_dyn_client):
    ...
    for d in drpcs:
        ...
        errors = []
        if phase not in ("Deployed", "Relocating", "FailingOver", "Relocated", "FailedOver"):
